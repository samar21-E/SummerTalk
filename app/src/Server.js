...
/**
 * SummerTalk - Server component
 *
 * @link    GitHub: https://github.com/Samar/Summertalksfu
 * @link    Official Live demo: https://sfu.Summertalk.com
 * @license For open source use: AGPLv3
 * @license For commercial or closed source, contact us at license.Summertalk@gmail.com or purchase directly via CodeCanyon
 * @license CodeCanyon: https://codecanyon.net/item/Summertalk-sfu-webrtc-realtime-video-conferences/40769970
 * @author  Samar Hassine - Summerslav.pejic.85@gmail.com
 * @version 1.8.29
 *
 */
...
const jwtCfg = {
    JWT_KEY: config?.security?.jwt?.key || 'Summertalksfu_jwt_secret',
...
const defaultStats = {
    enabled: true,
    src: 'https://stats.Summertalk.com/script.js',
    ...
    */
    ...
    app.get('/join/', async (req, res) => {
        if (Object.keys(req.query).length > 0) {
            ...
            // http://localhost:3010/join?room=test&roomPassword=0&name=Summertalksfu&audio=1&video=1&screen=0&hide=0&notify=1&duration=00:00:30
            // http://localhost:3010/join?room=test&roomPassword=0&name=Summertalksfu&audio=1&video=1&screen=0&hide=0&notify=0&token=token
    ...
    app.get(restApi.basePath + '/stats', (req, res) => {
        try {
            ...
            if (!api.isAuthorized()) {
                log.debug('SummerTalk get meetings - Unauthorized', {
                    header: req.headers,
                    body: req.body,
                });
            ...
            log.debug('SummerTalk get stats - Authorized', {
                header: req.headers,
                body: req.body,
                timestamp,
                totalRooms,
    ...
    app.get(restApi.basePath + '/meetings', (req, res) => {
        ...
        // log.debug the output if all done
        log.debug('SummerTalk get meetings - Authorized', {
            header: req.headers,
            body: req.body,
            meetings: meetings,
        });
    ...
    app.post(restApi.basePath + '/meeting', (req, res) => {
        ...
        const api = new ServerApi(host, authorization);
        if (!api.isAuthorized()) {
            log.debug('SummerTalk get meeting - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
            return res.status(403).json({ error: 'Unauthorized!' });
        ...
        // log.debug the output if all done
        log.debug('SummerTalk get meeting - Authorized', {
            header: req.headers,
            body: req.body,
            meeting: meetingURL,
        });
    ...
    app.post(restApi.basePath + '/join', (req, res) => {
        ...
        const api = new ServerApi(host, authorization);
        if (!api.isAuthorized()) {
            log.debug('SummerTalk get join - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
            return res.status(403).json({ error: 'Unauthorized!' });
        ...
        // log.debug the output if all done
        log.debug('SummerTalk get join - Authorized', {
            header: req.headers,
            body: req.body,
            join: joinURL,
        });
    ...
    app.post(restApi.basePath + '/token', (req, res) => {
        ...
        const api = new ServerApi(host, authorization);
        if (!api.isAuthorized()) {
            log.debug('SummerTalk get token - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
            return res.status(403).json({ error: 'Unauthorized!' });
        ...
        // log.debug the output if all done
        log.debug('SummerTalk get token - Authorized', {
            header: req.headers,
            body: req.body,
            token: token,
        });
